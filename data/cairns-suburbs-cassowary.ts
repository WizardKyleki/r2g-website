import type { CairnsSuburb } from "./suburbs";

export const cairnsCassowarySuburbs: CairnsSuburb[] = [
  {
    slug: "mission-beach",
    name: "Mission Beach",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "140km south",
    postcode: "4852",
    latitude: -17.8668,
    longitude: 146.1068,
    uniquePara1:
      "Mission Beach is one of North Queensland's most spectacular coastal communities, strung across four villages — Bingil Bay, Mission Beach, Wongaling Beach and South Mission Beach — backed by World Heritage Wet Tropics rainforest and fronted by the Coral Sea. Our team regularly helps families and retirees relocate to and from this stunning stretch of tropical coastline.",
    uniquePara2:
      "The area is one of the last known strongholds for the endangered southern cassowary, and rainforest surrounds many residential properties here. Our drivers are experienced navigating the winding access roads through Cassowary Coast towns and take particular care on unsealed driveways and properties backing onto dense vegetation.",
    uniquePara3:
      "The Bruce Highway turnoff at El Arish adds roughly 20 minutes of coastal road driving to reach Mission Beach. Our team factors this into scheduling and departs Cairns early to avoid peak-hour delays near Innisfail. Cyclone season brings the risk of flooding on low-lying access roads between November and April — we monitor road conditions closely and will reschedule when the Cassowary Coast road network is unsafe.",
    nearbySubs: ["Wongaling Beach", "Bingil Bay", "El Arish", "Tully"],
    priceFrom: "$179/hr",
    serviceArea: "Mission Beach & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle:
      "Removalists Mission Beach | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Mission Beach with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved our 3-bedroom home from Mission Beach to Cairns City. Despite the long Bruce Highway run and a tight rainforest driveway at our old place, the team arrived on time and had everything loaded without a scratch. Brilliant service.",
        name: "Leanne P.",
        location: "Mission Beach",
        date: "February 2025",
      },
      {
        text: "We relocated from Townsville to Mission Beach and R2G coordinated the whole thing from Cairns. The cassowary warning signs near our new property made us nervous about the driveway but the crew knew exactly what to expect and took real care.",
        name: "Mark & Julie T.",
        location: "Mission Beach",
        date: "November 2024",
      },
      {
        text: "Downsized from a large beach house in Mission Beach to a smaller unit in Innisfail. R2G were upfront about the drive time, quoted accurately and had us settled in by mid-afternoon. Couldn't have asked for better.",
        name: "Elaine H.",
        location: "Mission Beach",
        date: "August 2024",
      },
    ],
    tips: [
      {
        heading: "Allow extra travel time from Cairns.",
        text: "Mission Beach is approximately 140km south of Cairns via the Bruce Highway, with an additional 20 minutes on the coastal road from El Arish. We schedule an early departure to arrive on time — having everything packed and ready before our truck arrives helps the day run smoothly.",
      },
      {
        heading: "Check access roads in the wet season.",
        text: "Low-lying roads around Mission Beach and the four villages can flood quickly during heavy rain between November and April. If a cyclone or intense low is forecast in the days around your move, contact us early so we can discuss rescheduling options before road conditions deteriorate.",
      },
      {
        heading: "Watch for cassowaries near rainforest properties.",
        text: "Cassowaries occasionally cross driveways and residential streets, particularly in the early morning. Inform our team if you regularly see cassowaries near your property so our drivers can be alert — safety for both our crew and the local wildlife is a priority.",
      },
      {
        heading: "Use sturdy boxes for humidity and tropical conditions.",
        text: "Mission Beach's high humidity and frequent coastal showers can weaken cardboard boxes left outside. Have items packed in sealed boxes and kept undercover until our truck arrives. Plastic tubs are worth the investment for kitchen goods and linen in this climate.",
      },
    ],
  },
  {
    slug: "tully",
    name: "Tully",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "150km south",
    postcode: "4854",
    latitude: -17.9350,
    longitude: 145.9230,
    uniquePara1:
      "Tully is famously the wettest town in Australia, surrounded by extensive banana and sugar cane plantations that define the landscape of the Cassowary Coast hinterland. The town sits at the base of Mount Tyson and serves as a key service hub for the farming communities spread across the Tully River valley.",
    uniquePara2:
      "The farming character of Tully means many residents have large rural properties, sheds full of equipment and substantial outdoor furniture built for the tropics. Our team is equipped for agricultural-area moves, including navigating plantation access roads and loading oversized rural items.",
    uniquePara3:
      "The Bruce Highway runs through the centre of Tully and is the main artery connecting the town to Cairns, roughly 150km to the north. Our drivers know the seasonal conditions on this stretch — including the risk of cane-truck traffic during harvest season (June to November) — and plan departure times accordingly. Wet season flooding on the flats south of town can occasionally cut off Cardwell-direction routes, which we monitor closely for moves in that direction.",
    nearbySubs: ["Mission Beach", "El Arish", "Cardwell", "Tully Heads"],
    priceFrom: "$179/hr",
    serviceArea: "Tully & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle: "Removalists Tully | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Tully with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved us from a banana farm on the outskirts of Tully into a house in town. The track to our old place was muddy but the crew came prepared with matting and got the truck in without any dramas. Great job.",
        name: "Dean V.",
        location: "Tully",
        date: "January 2025",
      },
      {
        text: "Relocated from Cairns to Tully for a farm management role. R2G knew the Bruce Highway well, arrived exactly when they said and had our 4-bedroom home unloaded before the afternoon storms rolled in. Very professional.",
        name: "Sandra K.",
        location: "Tully",
        date: "October 2024",
      },
      {
        text: "Moved our family out of Tully to a place in Innisfail. The R2G team were friendly, efficient and handled our large outdoor furniture and NQ-built timber dining set with real care. Would definitely use them again.",
        name: "Rob & Tracey M.",
        location: "Tully",
        date: "July 2024",
      },
    ],
    tips: [
      {
        heading: "Plan around sugar cane harvest season.",
        text: "From June to November, heavy cane trucks operate extensively on roads around Tully, including sections of the Bruce Highway. These wide loads can slow traffic significantly. We schedule moves to avoid the busiest cane-road periods — let us know your preferred dates and we'll advise the best windows.",
      },
      {
        heading: "Expect rain any time of year.",
        text: "Tully averages over 4,000mm of rain annually — the highest of any Australian town. Even outside cyclone season, afternoon storms are frequent. Have everything packed and inside your home ready to load quickly, and keep a covered area near the truck for staging items between downpours.",
      },
      {
        heading: "Allow extra time for rural property access.",
        text: "Many Tully-area properties are on unsealed farm tracks off the main roads. Let us know your full address including the name of the access road — our drivers will check clearances and ground conditions before bringing the main truck in.",
      },
      {
        heading: "Wet season flooding can affect Cardwell road routes.",
        text: "The low-lying flats south of Tully towards Cardwell are vulnerable to flooding after intense rain. If your move involves southbound travel during the wet season, confirm road conditions with us 24 hours before your scheduled date.",
      },
    ],
  },
  {
    slug: "cardwell",
    name: "Cardwell",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "175km south",
    postcode: "4849",
    latitude: -18.2637,
    longitude: 146.0228,
    uniquePara1:
      "Cardwell is a quiet beachside town nestled between the Bruce Highway and the Coral Sea, with the spectacular peaks of Hinchinbrook Island visible from its foreshore. The town is the gateway to Hinchinbrook Island National Park and attracts fishing families, retirees and those seeking a relaxed tropical lifestyle well away from city bustle.",
    uniquePara2:
      "The Cardwell area was significantly impacted by Cyclone Yasi in 2011, and many rebuilt or renovated homes now feature modern cyclone-rated construction. Our team takes the extra care required when moving into and out of these properties, particularly with custom fittings and cyclone-compliant window and door systems.",
    uniquePara3:
      "At 175km from Cairns via the Bruce Highway, Cardwell is our southernmost regular service point in the region. Our team departs Cairns early to ensure a prompt arrival and plans the full day around the return drive. The stretch of Bruce Highway through the Cardwell Range is steep and winding — we use the right truck size for both the load and the road, ensuring safe transit through the range.",
    nearbySubs: ["Ingham", "Tully", "Tully Heads"],
    priceFrom: "$179/hr",
    serviceArea: "Cardwell & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle: "Removalists Cardwell | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Cardwell with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved us from Cardwell to a retirement property near Innisfail. Given the long drive, I wasn't sure how it would go — but the team were punctual, professional and had everything delivered in perfect condition. Really happy.",
        name: "Noel & Dawn C.",
        location: "Cardwell",
        date: "March 2025",
      },
      {
        text: "We moved from Townsville to Cardwell and R2G Cairns handled it beautifully. Knowing the Bruce Highway conditions well made a real difference — they timed arrival perfectly around the range drive and the afternoon heat.",
        name: "Jason F.",
        location: "Cardwell",
        date: "December 2024",
      },
      {
        text: "Relocated from Cardwell to Cairns City. R2G fit all our furniture into one trip — including a large outdoor setting and fishing gear — and still arrived at our new place before 2pm. Very efficient.",
        name: "Kylie B.",
        location: "Cardwell",
        date: "September 2024",
      },
    ],
    tips: [
      {
        heading: "Book an early start for long-distance moves.",
        text: "Cardwell is 175km from Cairns. To complete the move in a single day we depart early — typically around 6am. Confirm your preferred start time with us and make sure everything is packed and ready before our truck arrives to avoid delays.",
      },
      {
        heading: "The Cardwell Range requires the right truck.",
        text: "The Bruce Highway climbs steeply through the Cardwell Range section. Overloaded or poorly-maintained trucks can overheat or lose braking capacity on this descent. We ensure every truck is properly loaded and serviced before long-distance Cassowary Coast runs.",
      },
      {
        heading: "Protect cyclone-rated fittings.",
        text: "Post-Yasi homes in Cardwell often have high-value cyclone-compliant windows, shutters and door systems. Let us know about any fixed fittings or custom joinery you're moving so we can bring the right crating and wrapping materials.",
      },
      {
        heading: "Watch for wet season route closures.",
        text: "Flooding can affect sections of the Bruce Highway and local roads in and around Cardwell during heavy rain. We check conditions the evening before long-distance moves and will contact you if rescheduling is necessary for safety.",
      },
    ],
  },
  {
    slug: "ingham",
    name: "Ingham",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "220km south",
    postcode: "4850",
    latitude: -18.6501,
    longitude: 146.1621,
    uniquePara1:
      "Ingham is the heart of the Herbert River sugar cane district and the largest town in the Hinchinbrook Shire, known for its strong Italian heritage and its sprawling cane fields that stretch to the coast. The town serves as a regional centre for the broad agricultural community across the lower Cassowary Coast.",
    uniquePara2:
      "Sugar cane farming dominates the land around Ingham, and many properties in the district are associated with the industry — from town residences occupied by cane farming families to mill worker housing and rural leasehold properties. Our team is experienced with agricultural community moves and understands the practical demands of rural Far North Queensland relocations.",
    uniquePara3:
      "At 220km from Cairns, Ingham is the furthest point we regularly service on the Cassowary Coast. Our team treats these moves as full-day operations, departing Cairns before dawn and working efficiently to complete loading and delivery within a single day. The Bruce Highway is the only practical route, and we schedule around the cane harvest season (June to November) when heavy haulage traffic on this corridor is at its peak.",
    nearbySubs: ["Cardwell", "Tully"],
    priceFrom: "$179/hr",
    serviceArea: "Ingham & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle: "Removalists Ingham | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Ingham with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved our 4-bedroom home from Ingham to Cairns City. It was a huge day but the crew started early, worked non-stop and had us fully unloaded before dark. Incredible effort on a very long run.",
        name: "Tony & Maria S.",
        location: "Ingham",
        date: "February 2025",
      },
      {
        text: "We relocated from Townsville to Ingham and R2G Cairns handled the north-end delivery perfectly. They coordinated the timing, knew the route well and delivered everything without a mark on any piece.",
        name: "Linda G.",
        location: "Ingham",
        date: "October 2024",
      },
      {
        text: "Moved our parents out of their long-time Ingham home into a retirement property near Cairns. R2G were patient, gentle with the older furniture and made what could have been a stressful day feel very manageable.",
        name: "Paul R.",
        location: "Ingham",
        date: "July 2024",
      },
    ],
    tips: [
      {
        heading: "Expect a full-day move from Cairns.",
        text: "Ingham is 220km from our Cairns base. All Ingham moves are full-day operations. We depart well before dawn to ensure we arrive with enough time for a complete load and delivery in a single day. Have everything packed and ready for our early arrival.",
      },
      {
        heading: "Plan around the cane harvest.",
        text: "During the sugar cane harvest from June to November, the Bruce Highway south of Innisfail carries very heavy cane truck traffic — particularly in the early morning and late afternoon. We adjust our Ingham departure times to avoid the worst of this congestion.",
      },
      {
        heading: "Rural and farm property access needs early notice.",
        text: "Many Ingham-area properties are on rural roads or cane-farm access tracks. Give us your full property address well in advance so we can check route access and choose the right truck. We may use a smaller vehicle with shuttle runs for properties on rough or narrow farm roads.",
      },
      {
        heading: "Check for flood closures in the wet season.",
        text: "The Herbert River valley around Ingham is one of the most flood-prone areas in Queensland during the wet season. Road closures can affect access from both Cardwell and Townsville directions. We closely monitor the Queensland Traffic road conditions map before every Ingham move and will contact you if conditions require rescheduling.",
      },
    ],
  },
  {
    slug: "silkwood",
    name: "Silkwood",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "125km south",
    postcode: "4856",
    latitude: -17.7528,
    longitude: 145.9928,
    uniquePara1:
      "Silkwood is a small farming community on the Cassowary Coast, surrounded by banana plantations and sugar cane fields between Innisfail and Mission Beach. The town has a close-knit agricultural character, with many residents working the fertile volcanic soil of the Johnstone River district.",
    uniquePara2:
      "Properties in and around Silkwood often include farm sheds, fruit storage facilities and rural outbuildings alongside the main residence. Our team is experienced with moves that involve agricultural equipment, large outdoor items and the practical realities of rural property access on the Cassowary Coast.",
    uniquePara3:
      "The Bruce Highway passes close to Silkwood and is the main route for Cairns-bound moves, approximately 125km north. During banana harvest periods, the local road network sees increased small-truck traffic moving fruit between packing sheds and the highway. Our team works around these patterns and communicates clearly on move-day timing to keep your relocation on schedule through any delays.",
    nearbySubs: ["Mourilyan", "El Arish", "Mission Beach", "South Johnstone"],
    priceFrom: "$179/hr",
    serviceArea: "Silkwood & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle: "Removalists Silkwood | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Silkwood with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved us from a banana farm near Silkwood into a house in Innisfail. They navigated the farm tracks carefully, loaded everything including some heavy packing shed equipment and had us settled in by afternoon. Great work.",
        name: "Darren W.",
        location: "Silkwood",
        date: "January 2025",
      },
      {
        text: "Relocated from Cairns City out to our new place near Silkwood. R2G knew the area well, found the property without any issues and unloaded quickly despite the long driveway. Really professional service.",
        name: "Heather L.",
        location: "Silkwood",
        date: "November 2024",
      },
      {
        text: "Moved our 3-bedroom home from Silkwood to Tully. Short trip but the rural access at both ends needed careful truck handling. The R2G crew knew exactly what to do and were brilliant from start to finish.",
        name: "Brett & Anna F.",
        location: "Silkwood",
        date: "August 2024",
      },
    ],
    tips: [
      {
        heading: "Let us know about farm tracks and unsealed access.",
        text: "Many Silkwood properties are reached via unsealed farm roads or driveways through banana or cane paddocks. Advise us of your full access route when booking — we'll select the right vehicle and bring equipment for soft or uneven ground conditions.",
      },
      {
        heading: "Secure loose fruit and farm goods before moving day.",
        text: "If you have produce, chemicals or farming materials being moved, ensure everything is properly sealed and secured in appropriate containers before our team arrives. Loose bulk items are best consolidated into lidded bins or crates to protect your other belongings.",
      },
      {
        heading: "Wet season roads can become very soft.",
        text: "Unsealed farm roads around Silkwood can become impassable after heavy rain. If your property uses an unsealed access road, check conditions the evening before your move and contact us early if ground conditions are deteriorating.",
      },
      {
        heading: "Bruce Highway travel time allows for an efficient move day.",
        text: "Silkwood is 125km from Cairns on the Bruce Highway — a manageable drive that allows us to complete most residential moves in a single day. An early start from Cairns ensures we arrive by mid-morning, leaving plenty of daylight for loading and delivery.",
      },
    ],
  },
  {
    slug: "mourilyan",
    name: "Mourilyan",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "95km south",
    postcode: "4858",
    latitude: -17.5850,
    longitude: 146.0380,
    uniquePara1:
      "Mourilyan is a small coastal community just south of Innisfail, known for its deep-water port that serves as the primary export facility for the region's sugar cane production. The town sits alongside the Mourilyan Harbour and has a quiet, industrial-agricultural character shaped by its role in the sugar industry.",
    uniquePara2:
      "The surrounding cane district provides the backdrop for most residential properties here — homes are typically modest and practical, and many residents are connected directly or indirectly to the sugar mill and harbour operations. Our team understands the working community of Mourilyan and provides efficient, no-fuss removals suited to the area.",
    uniquePara3:
      "Mourilyan sits on the Kennedy Highway junction near Innisfail, approximately 95km south of Cairns on the Bruce Highway — placing it within a comfortable single-day move range. The local road network around the harbour precinct involves some narrow sections and industrial vehicle traffic related to the crushing season. We schedule Mourilyan moves with these local conditions in mind and time arrivals to avoid peak harbour-vehicle periods.",
    nearbySubs: ["South Johnstone", "Silkwood", "Mission Beach"],
    priceFrom: "$179/hr",
    serviceArea: "Mourilyan & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle:
      "Removalists Mourilyan | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Mourilyan with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved our family from Mourilyan into a new home in Innisfail. They knew the harbour roads well and timed arrival to avoid the mill trucks. Efficient, friendly and everything arrived in great condition.",
        name: "Steven B.",
        location: "Mourilyan",
        date: "February 2025",
      },
      {
        text: "Relocated from Cairns to Mourilyan for a port operations role. R2G handled the move quickly and professionally — a good price for the distance involved and the team were genuinely easy to work with.",
        name: "Craig N.",
        location: "Mourilyan",
        date: "October 2024",
      },
      {
        text: "Small house move within the Mourilyan area. Even for a compact job, R2G showed up fully equipped and treated our belongings with real care. Worth every dollar.",
        name: "Pam D.",
        location: "Mourilyan",
        date: "July 2024",
      },
    ],
    tips: [
      {
        heading: "Avoid industrial traffic near the harbour.",
        text: "Heavy vehicles servicing the Mourilyan Harbour operate on local roads throughout the crushing season (approximately June to November). We schedule moves to avoid the busiest morning periods around the port precinct and will advise the best arrival window when you book.",
      },
      {
        heading: "Proximity to Innisfail makes same-day moves easy.",
        text: "Mourilyan is just south of Innisfail and 95km from Cairns — well within our standard single-day service range. There's no need to split your move across multiple days for typical residential relocations.",
      },
      {
        heading: "Wet season flooding can affect harbour-area roads.",
        text: "Low-lying roads near Mourilyan Creek and the harbour can be affected by flooding during intense wet season rain. If your property is in a low-lying street, let us know and we will confirm road conditions before departing Cairns on your move day.",
      },
      {
        heading: "Pack to protect items from coastal humidity.",
        text: "Mourilyan's harbour-side location means elevated humidity and occasional salt-laden breezes. Metal tools, electronics and timber furniture are best wrapped in moisture-barrier materials for the move — our team brings appropriate wrapping for all types of belongings.",
      },
    ],
  },
  {
    slug: "south-johnstone",
    name: "South Johnstone",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "100km south",
    postcode: "4859",
    latitude: -17.6147,
    longitude: 145.9775,
    uniquePara1:
      "South Johnstone is a small agricultural community in the Johnstone River valley, surrounded by banana plantations that produce a significant proportion of Australia's banana crop. The community is tightly connected to the fruit industry, with many residents working in growing, packing and transport operations across the local plantations.",
    uniquePara2:
      "Banana farming defines the character of South Johnstone — packing sheds, irrigation infrastructure and farm machinery are common features on local properties. Our team is well-versed in moves involving rural properties with agricultural outbuildings and can handle practical farm relocation requirements efficiently.",
    uniquePara3:
      "South Johnstone sits just inland from the Bruce Highway near Innisfail, making it approximately 100km from Cairns and an efficient move destination. The local road network connects the community to the main highway via flat cane-country roads that are generally easy to navigate but can flood quickly when the Johnstone River rises during the wet season. We track river levels and road closures before every Cassowary Coast move.",
    nearbySubs: ["Mourilyan", "Silkwood", "El Arish"],
    priceFrom: "$179/hr",
    serviceArea: "South Johnstone & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle:
      "Removalists South Johnstone | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in South Johnstone with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved us from a banana farm property in South Johnstone to Cairns. They handled a large packing shed clearance on top of the household contents without any fuss. Great team, great price.",
        name: "Kevin O.",
        location: "South Johnstone",
        date: "March 2025",
      },
      {
        text: "Relocated from Innisfail to South Johnstone for farm management work. R2G were professional and knew the area roads well. The quote was accurate and the move went exactly to plan.",
        name: "Amanda R.",
        location: "South Johnstone",
        date: "November 2024",
      },
      {
        text: "Moved our 3-bedroom home from South Johnstone to Mission Beach. Short drive but the B-road access needed careful truck navigation. The R2G crew handled it well and everything arrived without a mark.",
        name: "Gary & Fiona L.",
        location: "South Johnstone",
        date: "September 2024",
      },
    ],
    tips: [
      {
        heading: "Monitor Johnstone River levels during the wet.",
        text: "South Johnstone sits in the Johnstone River floodplain. After heavy rain, road access to some properties can become compromised quickly. We check river levels and the Queensland Traffic road closure map before every move and will reschedule if access roads are at risk.",
      },
      {
        heading: "Farm properties need specific truck planning.",
        text: "Banana farm properties often have multiple buildings spread across the block. Let us know which items are at which location — main house, packing shed, machinery shed — so we can plan the most efficient loading sequence and avoid multiple repositioning runs.",
      },
      {
        heading: "Banana packing equipment needs careful handling.",
        text: "If you have overhead rail systems, packing tables or grading machinery being relocated, these need to be included in your quote request. Our team can handle most farm equipment disassembly but needs advance notice to bring the right tools and ratchet straps.",
      },
      {
        heading: "Inland location means slightly lower humidity.",
        text: "South Johnstone is a few kilometres inland from the coast, which can mean slightly less salt-air exposure than coastal properties. However, the valley still experiences very high humidity during the wet season — use moisture-barrier wrapping for electronics, books and leather items.",
      },
    ],
  },
  {
    slug: "el-arish",
    name: "El Arish",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "130km south",
    postcode: "4855",
    latitude: -17.8147,
    longitude: 146.0135,
    uniquePara1:
      "El Arish is a small agricultural township on the Cassowary Coast, sitting at the junction of the Bruce Highway and the road to Mission Beach. The town is surrounded by sugar cane and banana farming country and serves as a gateway community for those heading to the coastal villages of the Mission Beach area.",
    uniquePara2:
      "The community of El Arish is closely tied to the surrounding cane district, and many properties are either farming or rural residential. The town also attracts those seeking a quiet lifestyle within easy reach of Mission Beach's beaches and the World Heritage Wet Tropics rainforest.",
    uniquePara3:
      "El Arish's position on the Bruce Highway junction makes it a practical access point for removals heading deeper into the Mission Beach corridor. Our team uses El Arish as a reference point on Cassowary Coast moves and knows the surrounding road network well, including which roads become inaccessible during wet season flooding on the adjacent agricultural flats. The 130km Bruce Highway run from Cairns is straightforward for a well-planned early-start move.",
    nearbySubs: ["Mission Beach", "Silkwood", "Tully", "Bingil Bay"],
    priceFrom: "$179/hr",
    serviceArea: "El Arish & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle: "Removalists El Arish | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in El Arish with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved our family from El Arish to a new home in Cairns North. The team knew the Bruce Highway well, arrived early and had the truck loaded before the midday heat kicked in. Exceptional efficiency.",
        name: "Matthew J.",
        location: "El Arish",
        date: "January 2025",
      },
      {
        text: "Relocated from Townsville to El Arish and R2G coordinated everything from the Cairns end. Smooth, professional and the furniture arrived in perfect condition despite the long drive.",
        name: "Robyn C.",
        location: "El Arish",
        date: "November 2024",
      },
      {
        text: "Short move from El Arish out to a cane farm property nearby. Even for a small job R2G were professional and careful. Great local knowledge of the back roads too.",
        name: "Trevor H.",
        location: "El Arish",
        date: "August 2024",
      },
    ],
    tips: [
      {
        heading: "El Arish is a key junction — plan your route clearly.",
        text: "The Bruce Highway junction at El Arish connects to Mission Beach, Tully and beyond. Let us know your exact delivery address when booking so we can plan the most direct approach and avoid unnecessary backtracking on rural roads.",
      },
      {
        heading: "Agricultural traffic peaks during harvest seasons.",
        text: "During sugar cane harvest (June to November) and banana pick periods, local roads around El Arish carry higher agricultural vehicle traffic. We schedule around the busiest times and advise departure windows that keep your move running smoothly.",
      },
      {
        heading: "Wet season flooding affects low-lying access roads.",
        text: "Farm roads on the flat agricultural land around El Arish can flood after heavy rain. If your property uses a low-lying or unsealed access route, confirm conditions with us the day before your move so we can plan accordingly.",
      },
      {
        heading: "130km from Cairns suits a well-planned single-day move.",
        text: "El Arish is approximately 130km from our Cairns base — a manageable distance that allows a complete single-day residential move with an early start. Have everything packed and accessible before our arrival to maximise your moving day.",
      },
    ],
  },
  {
    slug: "kurrimine-beach",
    name: "Kurrimine Beach",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "120km south",
    postcode: "4871",
    latitude: -17.7720,
    longitude: 146.1200,
    uniquePara1:
      "Kurrimine Beach is a small, peaceful coastal community on the Cassowary Coast, known for its pristine swimming beaches, productive coral reef fishing and a relaxed lifestyle far removed from city pressures. The town attracts retirees, fishing enthusiasts and those seeking an authentic Far North Queensland coastal experience.",
    uniquePara2:
      "The residential character of Kurrimine Beach is dominated by holiday homes, fishing shacks and permanent residences set back from the beachfront. Properties here tend to be low-set or raised timber homes built for tropical living, and many include boat storage, fishing gear and outdoor entertaining areas that require thoughtful packing and loading.",
    uniquePara3:
      "Kurrimine Beach is accessed via a sealed road from the Bruce Highway, adding approximately 15 minutes to the drive from Cairns — 120km away. The coastal approach road passes through cane farming country and can flood at creek crossing points during intense wet season rain. Our team monitors these conditions before every Cassowary Coast coastal move and plans arrival windows that avoid the worst of the afternoon storm season.",
    nearbySubs: ["El Arish", "Mission Beach", "Silkwood", "Bingil Bay"],
    priceFrom: "$179/hr",
    serviceArea: "Kurrimine Beach & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle:
      "Removalists Kurrimine Beach | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Kurrimine Beach with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved us from a fishing shack at Kurrimine Beach to a house in Innisfail. They handled all the boat gear and outdoor fishing equipment with care and loaded everything efficiently. Really happy with the service.",
        name: "Phil & Joyce T.",
        location: "Kurrimine Beach",
        date: "February 2025",
      },
      {
        text: "Relocated from Cairns to our retirement home at Kurrimine Beach. R2G knew the coastal road well and arrived exactly on time. The team were friendly, fast and treated our furniture with real respect.",
        name: "Ian M.",
        location: "Kurrimine Beach",
        date: "December 2024",
      },
      {
        text: "Moved our holiday home contents from Kurrimine Beach to Cairns City. Even with the distance involved it was a smooth and well-run day. The crew were excellent and the quote was very fair.",
        name: "Laura G.",
        location: "Kurrimine Beach",
        date: "September 2024",
      },
    ],
    tips: [
      {
        heading: "Coastal road access needs wet-weather monitoring.",
        text: "The road into Kurrimine Beach passes through low-lying cane country with creek crossings that can flood after heavy rain. We check road conditions before departing Cairns and will contact you if we anticipate access difficulties on your move day.",
      },
      {
        heading: "Holiday homes may need extra packing preparation.",
        text: "If you are clearing a holiday property, ensure all loose outdoor items, fishing gear and boat accessories are consolidated and ready to load. We can help you estimate volume for a quote if you describe your full inventory in advance.",
      },
      {
        heading: "Boat and fishing equipment needs proper securing.",
        text: "Fishing rods, reels, tackle boxes and marine accessories are fragile and easy to lose in a general move. Pack small items into clearly labelled sealed containers before our team arrives — we can advise the best approach for larger items like tinnies and canoes.",
      },
      {
        heading: "Salt air can affect timber and metal items.",
        text: "Beachside properties at Kurrimine Beach expose timber furniture and metal fittings to ongoing salt air. Point out any items with rust or timber weathering so our team can use extra padding and moisture protection during transit.",
      },
    ],
  },
  {
    slug: "bingil-bay",
    name: "Bingil Bay",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "135km south",
    postcode: "4852",
    latitude: -17.8350,
    longitude: 146.1050,
    uniquePara1:
      "Bingil Bay is the northernmost of the Mission Beach villages and arguably the most tranquil, nestled between World Heritage-listed Wet Tropics rainforest and the Coral Sea. The bay is one of the best remaining habitats for the endangered southern cassowary, and dense rainforest backs directly onto residential streets.",
    uniquePara2:
      "The intimate character of Bingil Bay means properties are often set within lush tropical gardens with narrow access paths and mature rainforest canopy close to the road. Our team navigates these environments carefully, respecting both the properties and the wildlife habitat that surrounds them.",
    uniquePara3:
      "Bingil Bay is reached via the Mission Beach road from the Bruce Highway at El Arish, approximately 135km from Cairns. The entire coastal corridor through the Mission Beach villages is a narrow, winding road through dense vegetation — unsuitable for large articulated trucks. We use appropriately-sized vehicles for all Bingil Bay moves and plan vehicle access to each property before the move day to avoid any on-the-day surprises.",
    nearbySubs: ["Mission Beach", "Wongaling Beach", "El Arish"],
    priceFrom: "$179/hr",
    serviceArea: "Bingil Bay & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle: "Removalists Bingil Bay | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Bingil Bay with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved us from Bingil Bay to Cairns City. The narrow road through the village needed careful truck selection but the team came fully prepared with the right vehicle. Everything was handled brilliantly.",
        name: "Elise & Tom W.",
        location: "Bingil Bay",
        date: "January 2025",
      },
      {
        text: "Relocated from Cairns to our new home in Bingil Bay. We were worried about the tight road access but R2G had clearly scoped the route in advance and executed the move perfectly without disturbing the rainforest surrounds.",
        name: "Karen F.",
        location: "Bingil Bay",
        date: "October 2024",
      },
      {
        text: "Small move from Bingil Bay to Mission Beach village. The R2G crew were professional, careful in the cassowary habitat around our garden and had everything done quickly. Couldn't be happier.",
        name: "Michael S.",
        location: "Bingil Bay",
        date: "July 2024",
      },
    ],
    tips: [
      {
        heading: "Vehicle size matters for the Mission Beach corridor.",
        text: "The road through Bingil Bay and the Mission Beach villages is narrow and winding, with overhanging vegetation. Large trucks cannot safely access some properties here. Let us know your exact address when booking so we can select the right vehicle before your move day.",
      },
      {
        heading: "Cassowary awareness on every visit.",
        text: "Southern cassowaries are known to cross driveways and streets around Bingil Bay, particularly in the early morning and at dusk. Our drivers are fully briefed on cassowary behaviour and will slow or stop for any wildlife encountered near your property.",
      },
      {
        heading: "Rainforest humidity affects item condition quickly.",
        text: "Dense Wet Tropics vegetation creates extremely high ambient humidity around Bingil Bay properties. Books, artworks, leather goods and unprotected electronics deteriorate quickly in these conditions — use sealed plastic storage and moisture absorbers, especially during wet season moves.",
      },
      {
        heading: "Cyclone preparation is important in this location.",
        text: "The Mission Beach area is one of the most frequently cyclone-impacted areas of Queensland. If a cyclone watch is issued in the days before your move, contact us immediately so we can reassess timing and plan for the safest possible outcome for your belongings.",
      },
    ],
  },
  {
    slug: "tully-heads",
    name: "Tully Heads",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "155km south",
    postcode: "4854",
    latitude: -18.0160,
    longitude: 146.0450,
    uniquePara1:
      "Tully Heads is a quiet coastal settlement at the mouth of the Tully River, known for its waterway access, fishing, and a lifestyle built around the natural environment. The community has a genuine off-the-beaten-track character and attracts residents who value peace and proximity to coastal and river wilderness.",
    uniquePara2:
      "The road to Tully Heads leads from Tully through cane farming country before reaching the coast, and the settlement itself is small and spread out along the river mouth. Many properties here are modest holiday shacks or permanent rural residences with large yards and river-facing aspects.",
    uniquePara3:
      "Tully Heads sits approximately 155km from Cairns, adding a coastal access road beyond Tully to the Bruce Highway travel time. This access road passes through low-lying cane flats that are particularly vulnerable to flooding when the Tully River is running high during the wet season. Our team tracks Tully River conditions before every move to this location and maintains clear communication with clients if weather conditions require rescheduling.",
    nearbySubs: ["Tully", "Mission Beach", "Cardwell"],
    priceFrom: "$179/hr",
    serviceArea: "Tully Heads & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle:
      "Removalists Tully Heads | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Tully Heads with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved our fishing shack contents from Tully Heads to a house in Tully. They knew the river road well and had the right size truck for the access conditions. Everything went smoothly — great team.",
        name: "Barry H.",
        location: "Tully Heads",
        date: "March 2025",
      },
      {
        text: "Relocated from Cairns to Tully Heads for a lifestyle change. R2G were upfront about the access road conditions, arrived fully prepared and had us settled in before the afternoon storm. Very professional outfit.",
        name: "Donna V.",
        location: "Tully Heads",
        date: "November 2024",
      },
      {
        text: "Moved our holiday property at Tully Heads into storage in Cairns. The small access track was no problem for R2G — they clearly assessed it in advance and came with the right gear. Reliable and fair.",
        name: "Colin P.",
        location: "Tully Heads",
        date: "August 2024",
      },
    ],
    tips: [
      {
        heading: "The river road can flood quickly.",
        text: "The access road from Tully to Tully Heads crosses low-lying Tully River floodplain country. After significant rainfall the road can become unsafe within hours. We always check Tully River levels and local road conditions the evening before your move and will contact you immediately if rescheduling is needed.",
      },
      {
        heading: "Small trucks are best for coastal access roads.",
        text: "The road into Tully Heads is unsuitable for large removal vehicles in some sections. All Tully Heads moves use appropriately-sized trucks — this may mean a shuttle run for larger households, which we factor into your quote.",
      },
      {
        heading: "Allow a full day for Cairns-based moves.",
        text: "At 155km from Cairns including the coastal approach, Tully Heads moves are full-day operations. We depart early and plan for all contingencies — have your items packed and ready before our arrival to make the most of the available daylight.",
      },
      {
        heading: "Secure watercraft and river gear properly.",
        text: "Many Tully Heads residents have tinnies, kayaks and fishing equipment central to their lifestyle. Include all watercraft and marine accessories in your quote request so we can bring the right ratchet straps, padding and loading ramps for safe transit.",
      },
    ],
  },
  {
    slug: "wongaling-beach",
    name: "Wongaling Beach",
    region: "Cassowary Coast",
    state: "QLD",
    distanceFromCBD: "142km south",
    postcode: "4852",
    latitude: -17.8914,
    longitude: 146.1028,
    uniquePara1:
      "Wongaling Beach is the central village in the Mission Beach community, home to the area's main shops, cafes and services. The beach here is long and lined with coconut palms, and the village balances a tourist-friendly atmosphere with a strong permanent residential community drawn by the tropical lifestyle.",
    uniquePara2:
      "Wongaling Beach has a mix of beachfront holiday apartments, permanent family homes and elevated timber residences set in lush tropical gardens. Our team is experienced with the variety of property types found across the Mission Beach villages and handles the coastal move environment — including salt air, narrow access roads and sandy driveways — with the care it requires.",
    uniquePara3:
      "As the most central of the Mission Beach villages, Wongaling Beach is approximately 142km from Cairns via the Bruce Highway and Mission Beach Road. The village sits within one of Australia's most active cyclone impact zones — Cyclone Yasi crossed directly over this area in 2011 — and residents take cyclone season seriously. We monitor Bureau of Meteorology cyclone watches closely for every wet-season move along the Cassowary Coast.",
    nearbySubs: ["Mission Beach", "Bingil Bay", "El Arish", "Tully"],
    priceFrom: "$179/hr",
    serviceArea: "Wongaling Beach & Cassowary Coast",
    address: "36 Abbott St, Cairns City QLD 4870",
    phone: "1300 959 498",
    metaTitle:
      "Removalists Wongaling Beach | R2G Transport & Storage | From $179/hr",
    metaDescription:
      "Trusted removalists in Wongaling Beach with 10+ years experience. Local & Cairns moves from $179/hr. Fully insured, no hidden fees. Call 1300 959 498 or get a free quote online.",
    reviews: [
      {
        text: "R2G moved us from Wongaling Beach to a new home in Cairns. They navigated the coastal village roads perfectly, loaded our beachside home contents carefully and delivered everything to Cairns in great condition.",
        name: "Nathan & Sophie R.",
        location: "Wongaling Beach",
        date: "February 2025",
      },
      {
        text: "Relocated from Cairns City to Wongaling Beach. R2G knew the Mission Beach area roads well and timed the arrival perfectly. The crew were efficient, careful and a pleasure to deal with on the day.",
        name: "Jenny L.",
        location: "Wongaling Beach",
        date: "December 2024",
      },
      {
        text: "Moved between two properties in the Mission Beach villages. Even a short local move here benefits from professionals who know the narrow roads. R2G were excellent — on time, well-equipped and very tidy.",
        name: "Brian O.",
        location: "Wongaling Beach",
        date: "September 2024",
      },
    ],
    tips: [
      {
        heading: "Cyclone season requires flexible booking.",
        text: "Wongaling Beach sits in one of Australia's most active cyclone zones. Between November and April, we recommend having a backup date in mind when you book. If a cyclone watch is issued before your move date, contact us immediately so we can discuss options safely.",
      },
      {
        heading: "Narrow village roads limit truck sizes.",
        text: "The Mission Beach village road network, including streets around Wongaling Beach village centre and the esplanade, is narrow with limited turning space for large trucks. We select vehicle size based on your property address — let us know your full address at booking to ensure the right truck is allocated.",
      },
      {
        heading: "Beachfront properties need extra moisture protection.",
        text: "Salt air exposure is significant on properties close to the Wongaling Beach foreshore. Timber furniture, metal fixtures and electronics need moisture-barrier wrapping for any move. Our team brings the right materials — just let us know about any particularly sensitive items.",
      },
      {
        heading: "Book well ahead during the dry season.",
        text: "June to October is the most popular period for Cassowary Coast moves due to the reliable dry weather. Demand for removal services peaks during these months — book 3 to 4 weeks in advance to secure your preferred date, especially for weekend moves.",
      },
    ],
  },
];
